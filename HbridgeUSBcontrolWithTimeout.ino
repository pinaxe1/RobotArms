...
{ 
  for (int i=0; i<motors_n ;i++) {
    ...
    bool sign=dif>0;
    if (!motor[i].runs && abs(dif)> motor[i].precision ) // If needed & difference is big enough
       runMotor(motor[i],sign,200);                      // StartMotor 
    if (motor[i].runs && abs(dif)<= motor[i].precision ) // if needed & precision is reached. 
       stopMotor(motor[i]);                              // StopMotor 
...
{  unsigned long aTime =millis();
   ...
   for (int i=0; i<motors_n ;i++) {
      if ( motor[i].runs && aTime>=motor[i].stoptime )
           stopMotor(motor[i]);
      if (motor[i].runs) fin`ished=false;
      }       
   return(finished);   
};
